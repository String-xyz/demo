...
<ModalBase title="Complete checkout">
	...
	<div class="mt-6 text-center">
		...
		<button class="btn btn-primary rounded border-2 normal-case " on:click={payWithCard} disabled={geoRestricted || APIError}>Pay w/ Card</button>
		{#if geoRestricted}
			<div class="tooltip tooltip-primary tooltip-left" data-tip="Unfortunately, we cannot serve your state due to local laws.">
				<img src="/images/question_icon.svg" alt="question"/>
			</div>
		{:else if APIError}
			<div class="tooltip tooltip-primary tooltip-left" data-tip="Unfortunately, we are unable to contact our API at this moment.">
				<img src="/images/question_icon.svg" alt="question"/>
			</div>
		{/if}
	</div>
